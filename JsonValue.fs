...
type private JsonParser(jsonText: string) =
    ...
    let throw (msg: string) =
        failwith (sprintf "Invalid JSON starting at character %d: %s" i msg)
    ...
    and parseLiteral (expected, r) =
        ensure (i + expected.Length <= s.Length) (sprintf "expected `%s`" expected)

        for j in 0 .. expected.Length - 1 do
            ensure (s.[i + j] = expected.[j]) (sprintf "expected `%s`" expected)
