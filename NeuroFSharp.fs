...
type DeserError =
    ...
    override this.ToString() =
        ...
        sprintf "Error while deserializing field <root>%s: %s" path this.error
...
module DeserError =
    let internal unexpected (unexpected: JsonValue) (expected: string) (path: JsonPath) : DeserError =
        ...
        { path = path
          error = sprintf "unexpected %s, expected a %s" unexpected' expected }

    let internal case (unexpected: string) (expected: string array) (path: JsonPath) : DeserError =
        let sep = "\", \""
        let expected' = sprintf "\"%s\"" (String.concat sep expected)

        { path = path
          error = sprintf "unexpected value \"%s\", expected one of %s" unexpected expected' }
    ...
    let internal missingField (path: JsonPath) : DeserError =
        let field =
            ...
            | Prop x -> sprintf "field \"%s\"" x
            | Index x -> sprintf "item %d" x

        { path = List.tail path
          error = sprintf "missing %s" field }

    let internal exc (exc: string) (path: JsonPath) : DeserError =
        { path = path
          error = sprintf "caught exception: %s" exc }
...
module internal TypeInfo =
    ...
    and fromSystemType (ty: Type) : TypeInfo =
        ...
        (ty,
         ...
         | _ -> raise (NotImplementedException(sprintf "type info for type %s is not implemented" ty.FullName)))
...
type Game<'T>() =
    ...
    member this.Start(url: string option, ct: CancellationToken) : Task =
        ...
        let log x = Printf.ksprintf this.LogError x
        let log' x = Printf.ksprintf this.LogDebug x
                    ...
                    |> Result.bind (fun data ->
                        try
                            ...
                            | Ok(None) ->
                                ...
                                Error(Some(sprintf "Unknown action name: %s, expected one of: \"%s\"" name names))
                            | Error(err) -> Error(Some(sprintf "Invalid action data: %s" err.DisplayText))
                        with exc ->
                            log "%A" exc
                            Error(Some(sprintf "Unhandled exception: %s" exc.Message)))
        ...
        async {
                            ...
                            let rec mapCommand msg =
                                ...
                                | MsgRetain actions ->
                                    ...
                                    let unreg =
                                        registered
                                        |> Seq.map _.Key
                                        |> Seq.filter (new_reg.Contains >> not)
                                        |> List.ofSeq
                                ...
                                | MsgCmd(Startup game) ->
                                    log' "resetting registered actions"
                                        ...
                                        |> List.filter (fun x ->
                                            if not x.Valid then
                                                if registered.ContainsKey x.Name then
                                                    ...
                                                    log' "unregistering %s (invalid)" x.Name
                                                ...
                                                | Some y ->
                                                    ...
                                                    log' "reregistering %s" x.Name
                                                    true
                                                | None ->
                                                    registered <- Map.add x.Name (x.Clone()) registered
                                                    log' "registering %s" x.Name
                                                    true)
            ...
            while true do
                try
                    ...
                    do! Async.AwaitTask(ws1.ConnectAsync(Uri(url), ct))
                    log' "Connected to %s" url
                    ...
                    let read () =
                        async {
                            ...
                            let! res1 = Async.AwaitTask(ws1.ReceiveAsync(buf, ct))
                            ...
                            while ws1.State = WebSocketState.Open do
                                try
                                    ...
                                    | WebSocketMessageType.Close ->
                                        do! Async.AwaitTask(ws1.CloseAsync(WebSocketCloseStatus.NormalClosure, "", ct))
                                    | WebSocketMessageType.Text
                                    | WebSocketMessageType.Binary ->
                                        ...
                                        | Error(err) -> log "Invalid server command: %A" err
                                    | _ -> ()
                                with exc ->
                                    log "Unhandled receive exception: %A" exc
                        }
                    ...
                    with exc ->
                        log "Reregister failed: %A" exc

                    do! receive
                with exc ->
                    log "Connection failed: %A" exc

                do! Async.AwaitTask(Task.Delay(3000, ct))
        }
        |> (fun x -> Async.StartImmediateAsTask(x, ct))
